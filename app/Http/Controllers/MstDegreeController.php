...
class MstDegreeController extends Controller
{
...
/* Start */ 
	public function DegreeListAll()
	{
		...
		$result = Degree::select('mst_degree_id','mst_parent_id','mst_degree_detail')
			->where('mst_parent_id', '0')
			->where('mst_degree_status', '1')		
			->get();
		...
		if (count($result) != 0)
		{
			...
			foreach ($result as  $degree) {
				...
				$result1 = Degree::select('mst_degree_id','mst_parent_id','mst_degree_detail')
						->where('mst_parent_id', $degree['mst_degree_id'])
						->where('mst_degree_status', '1')										
						->get();				
				...
				if (count($result1) != 0)
				{
					...
					$result[] = array($degree['mst_degree_detail'] => $result1);
