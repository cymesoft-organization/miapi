...
/* Country For List All in Get Methode */
/* Start */ 
	public function CountryListAll()
	{

			
		$result = Country::select('mst_country_id','mst_parent_id','mst_country_detail')
		...
		if (count($result) != 0)
		{
			...
			foreach ($result as  $countryVal) {
								
			
				$result1 = Country::select('mst_country_id','mst_parent_id','mst_country_detail')
						->where('mst_parent_id', $countryVal['mst_country_id'])
						->where('mst_country_status', '1')										
						->get();				
				...
				if (count($result1) != 0)
				{
					...
					$result[] = array($countryVal['mst_country_detail'] => $result1);
